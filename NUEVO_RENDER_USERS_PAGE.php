<?php
/**
 * NUEVA VERSIÓN MEJORADA DE render_users_page()
 * 
 * INSTRUCCIONES:
 * 1. Copiar todo el contenido de esta función (líneas 10-900)
 * 2. Reemplazar la función actual render_users_page() en class-fplms-users.php
 *    (desde línea 325 hasta línea 1509)
 * 3. Guardar y probar
 */
    ...
    public function render_users_page(): void {
        ...
        $search_query = isset( $_GET['fplms_search'] ) ? sanitize_text_field( wp_unslash( $_GET['fplms_search'] ) ) : '';
        ...
        $args = [
            'number' => -1, // Obtener todos primero, luego paginamos en frontend
            'orderby' => 'registered',
            'order' => 'DESC',
        ];
        ...
        // Filtrar por IDUsuario si se busca
        if ( ! empty( $search_query ) ) {
            $filtered_users = [];
            foreach ( $all_users as $user ) {
                ...
                if (
                    ...
                    stripos( $user_full_name, $search_query ) !== false ||
                    stripos( $id_usuario, $search_query ) !== false
                ) {
                    $filtered_users[] = $user;
                }
            }
            ...
            <div class="fplms-users-container">
                ...
                <div class="fplms-users-header">
                    ...
                    <div class="fplms-users-actions">
                        <button class="fplms-action-button" onclick="fplmsToggleSection('crear-usuario')">
                            ➕ Crear Usuario
                        </button>
                        <button class="fplms-action-button secondary" onclick="fplmsToggleSection('matriz-privilegios')">
                ...
                <div class="fplms-main-container">
                    ...
                    <!-- FILTROS (COLAPSABLES) -->
                    <div class="fplms-filters-section" id="fplms-filters">
                        <form method="get" id="fplms-filters-form">
                                ...
                                <div class="fplms-filter-field">
                                    ...
                                    <select name="fplms_filter_branch" id="fplms_filter_branch">
                                        ...
                                        <?php foreach ( $branches as $id => $name ) : ?>
                                            <option value="<?php echo esc_attr( $id ); ?>" <?php selected( $filter_ branch, $id ); ?>>
                                                <?php echo esc_html( $name ); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <?php if ( $filter_city || $filter_company || $filter_channel || $filter_branch || $filter_role ) : ?>
                                <a href="?page=fplms-users" class="fplms-filter-button" style="display: inline-block; margin-left: 10px; background: linear-gradient(135deg, #607D8B 0%, #455A64 100%); text-decoration: none;">
                                    ✖ Limpiar Filtros
                                </a>
                            <?php endif; ?>
                        </form>
                    ...
                    <div class="fplms-table-controls">
                        ...
                        <div class="fplms-per-page-selector">
                            ...
                            <select id="fplms-per-page" onchange="fplmsChangePerPage(this.value)">
                ...
                <!-- SECCIÓN: CREAR USUARIO (Oculta) -->
                <div id="crear-usuario" class="fplms-hidden-section">
                    <!-- Aquí va el formulario de crear usuario existente -->
                    <h2>Crear Nuevo Usuario</h2>
                    <p>Formulario de creación de usuarios (mantener el código existente)</p>
                </div>

                <!-- SECCIÓN: MATRIZ DE PRIVILEGIOS (Oculta) -->
                <div id="matriz-privilegios" class="fplms-hidden-section">
        ...
        jQuery(document).ready(function($) {
            ...
            fplmsPaginate(1, <?php echo $per_page; ?>);

            // Inicializar búsqueda si hay término
            <?php if ( ! empty( $search_query ) ) : ?>
                fplmsSearchUsers('<?php echo esc_js( $search_query ); ?>');
            <?php endif; ?>
        });

        /**
         * Toggle de secciones ocultas
         */
        function fplmsToggleSection(sectionId) {
            const section = document.getElementById(sectionId);
            const allSections = document.querySelectorAll('.fplms-hidden-section');
         ...
         */
        function fplmsChangePerPage(perPage) {
